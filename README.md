...
## Docker Setup (Local Postgres)

This setup runs the API and a Postgres database via Docker Compose.

Edit the environment values in `docker-compose.yml` as needed (especially `JWT_SECRET` and S3 values).
...
The API will be available at http://localhost:3000. On first boot, migrations are applied automatically.
